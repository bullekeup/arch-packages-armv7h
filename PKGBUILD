...
pkgbase=opencv
pkgname=('opencv-gstreamer-face-rpitwo' 'opencv-gstreamer-face-rpitwo-samples')
pkgver=3.1.0
pkgrel=3
...
prepare() {
  cd "$srcdir/"$(echo $pkgname | sed -e s/-gstreamer-face-rpitwo//)"-$pkgver"
  ...
  sed -i -e "s/\(vtkRenderingOpenGL\)/\12/g" "$srcdir/"$(echo $pkgname | sed -e s/-gstreamer-face-rpitwo//)"-$pkgver/cmake/OpenCVDetectVTK.cmake"
}
...
package_opencv-gstreamer-face-rpitwo() {
  ...
  install -Dm644 "$srcdir/"$(echo $pkgname | sed -e s/-gstreamer-face-rpitwo//)"-$pkgver/LICENSE" \
    "$pkgdir/usr/share/licenses/"$(echo $pkgname | sed -e s/-gstreamer-face-rpitwo//)"/LICENSE"

  cd "$pkgdir/usr/share"

  # separate samples package; also be -R friendly
  if [[ -d OpenCV/samples ]]; then
    mv OpenCV/samples "$srcdir/"$(echo $pkgname | sed -e s/-gstreamer-face-rpitwo//)"-samples"
    mv OpenCV "$(echo $pkgname | sed -e s/-gstreamer-face-rpitwo//)" # otherwise folder naming is inconsistent
  elif [[ ! -d OpenCV ]]; then
    warning "Directory naming issue; samples package may not be built!"
  fi
}

package_opencv-gstreamer-face-rpitwo-samples() {
  provides=('opencv-samples' 'opencv-gstreamer-samples')
  conflicts=('opencv-samples' 'opencv-gstreamer-samples')
  pkgdesc+=" (samples)"
  depends=("opencv-gstreamer-face-rpitwo=$pkgver") # sample codes change with lib/API
  unset optdepends
